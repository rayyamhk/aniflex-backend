...
import { ValidateKeyPipe } from './pipe/validateKey.pipe';
...
export class ImagesController {
  ...
  async createImage(@UploadedFile(ValidateExistencePipe) image: Express.Multer.File) {
    const key = this.getHash();
    await this.storageService.put(key, image);
    return this.utilsService.formatResponse(`Image uploaded (key: ${key})`, { path: `/images/${key}` });
  }
